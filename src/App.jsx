...
const InboundExcel = () => {
  ...
  const COL = { name: ["tên sản phẩm", "tên album", "album", "name"], artist: ["nghệ sĩ", "nghe si", "artist", "ca sĩ", "ban nhạc", "ban nhac"], sku: ["sku", "mã sku"], barcode: ["barcode", "mã vạch"], desc: ["mô tả", "desc"], group: ["loại sản phẩm", "loại", "thể loại"], brand: ["nhãn hiệu", "brand", "hãng"], cost: ["giá nhập", "giá vốn", "cost"], price: ["giá bán", "giá bán lẻ", "price"] };
  const cols = ["Album *", "Nghệ sĩ", "Mã SKU", "Barcode", "Mô tả", "Loại sản phẩm", "Nhãn hiệu", "Giá nhập", "Giá bán"];
  const fk = (h, v) => h.find((x) => v.includes(String(x).trim().toLowerCase()));
  const parse = (file) => { if (!file || !XLSX) return; if (!/\.(xlsx|xls|csv)$/i.test(file.name)) return toast("Chọn file Excel!"); setLoading(true); setFileName(file.name); const r = new FileReader(); r.onload = (e) => { try { const wb = XLSX.read(e.target.result, { type: "array" }); const json = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval: "" }); if (!json.length) { toast("File rỗng!"); setLoading(false); return; } const H = Object.keys(json[0]); const K = {}; Object.entries(COL).forEach(([k, v]) => (K[k] = fk(H, v))); if (!K.name) { toast("Thiếu cột bắt buộc: Album"); setLoading(false); return; } let auto = products.length + 1; const P = [], E = []; json.forEach((row, i) => { const name = String(row[K.name] || "").trim(); const artist = K.artist ? String(row[K.artist] || "").trim() : ""; let sku = K.sku ? String(row[K.sku] || "").trim() : ""; const isAuto = !sku; if (!sku) sku = "SKU" + String(auto++).padStart(3, "0"); const rec = { id: sku, name, artist, brand: K.brand ? String(row[K.brand] || "").trim() : "", barcode: K.barcode ? String(row[K.barcode] || "").trim() : "", desc: K.desc ? String(row[K.desc] || "").trim() : "", group: K.group ? String(row[K.group] || GROUPS[0]).trim() : GROUPS[0], cost: Number(String(row[K.cost]).replace(/[^\d.-]/g, "")) || 0, price: Number(String(row[K.price]).replace(/[^\d.-]/g, "")) || 0, _row: i + 2, _valid: !!name, _auto: isAuto, _isNew: !products.find((p) => p.id === sku) }; P.push(rec); if (!name) E.push(`Dòng ${i + 2}: thiếu tên album`); }); setRows(P); setErrors(E); setLoading(false); toast(`Đã đọc ${P.length} dòng!`); } catch { toast("Lỗi đọc file!"); setLoading(false); } }; r.readAsArrayBuffer(file); };
  ...
  const dl = () => { if (!XLSX) return; const ws = XLSX.utils.json_to_sheet([{ "Album": "OK Computer", "Nghệ sĩ": "Radiohead", "Mã SKU": "", "Barcode": "8935101", "Mô tả": "Album 1997", "Loại sản phẩm": "Rock", "Nhãn hiệu": "Parlophone", "Giá nhập": 600000, "Giá bán": 990000 }, { "Album": "Kind of Blue", "Nghệ sĩ": "Miles Davis", "Mã SKU": "", "Barcode": "8935102", "Mô tả": "Album 1959", "Loại sản phẩm": "Jazz", "Nhãn hiệu": "Columbia", "Giá nhập": 550000, "Giá bán": 890000 }]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "DiaThan"); XLSX.writeFile(wb, "mau_import.xlsx"); toast("Đã tải file mẫu!"); };
  ...
  return <div className="max-w-5xl"><PageHead title="Nhập hàng mới (Excel)" desc="Mã SKU để trống → hệ thống tự cấp" /><Card className="p-5 mb-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end"><div className="flex items-end gap-2"><div className="flex-1"><Select label="Nhà cung cấp *" value={supplier} onChange={(e) => setSupplier(e.target.value)}><option value="">-- Chọn NCC --</option>{suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></div><Btn variant="outline" onClick={() => setShowAdd(true)}><Plus size={16} /></Btn></div><Select label="Chi nhánh nhập" value={br} onChange={(e) => setBr(e.target.value)}>{branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</Select></div></Card>
    ...
    {rows.length > 0 && <div className="mt-4"><Table head={["Dòng", "SKU", "Nghệ sĩ", "Album", "Nhãn hiệu", "Loại", "Giá bán", "Trạng thái"]}>{rows.map((r, i) => <Row key={i}><Td className="text-slate-400">{r._row}</Td><Td className="font-bold text-blue-600">{r.id} {r._auto && <span className="text-[10px] text-cyan-500">(auto)</span>}</Td><Td className="font-medium text-slate-700">{r.artist || "—"}</Td><Td>{r.name || "—"}</Td><Td className="text-slate-500">{r.brand || "—"}</Td><Td className="text-slate-500">{r.group}</Td><Td className="font-semibold">{fmt(r.price)}</Td><Td>{!r._valid ? <Badge cls="bg-rose-50 text-rose-600 ring-rose-200">Lỗi</Badge> : r._isNew ? <Badge cls="bg-cyan-50 text-cyan-600 ring-cyan-200">Tạo mới</Badge> : <Badge cls="bg-slate-100 text-slate-600 ring-slate-200">Cập nhật</Badge>}</Td></Row>)}</Table></div>}
