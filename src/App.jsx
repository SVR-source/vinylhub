...
const SaleNew = () => {
  ...
  const addLine = (sku) => { if (!sku || lines.find((l) => l.sku === sku)) return; const p = visProducts.find((x) => x.id === sku); setLines([...lines, { sku, qty: 1, price: p.price }]); };
  ...
  return <div className="max-w-4xl"><PageHead title="Tạo đơn hàng mới" /><Card className="p-5 sm:p-6 space-y-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><Select label="Khách hàng" value={cust} onChange={(e) => setCust(e.target.value)}>{customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</Select><Select label="Chi nhánh bán" value={br} onChange={(e) => setBr(e.target.value)}>{branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</Select></div><Select label="Thêm sản phẩm" value="" onChange={(e) => addLine(e.target.value)}><option value="">-- Chọn đĩa --</option>{visProducts.map((p) => <option key={p.id} value={p.id}>{p.name} (Tồn: {stock[`${p.id}-${br}`] || 0})</option>)}</Select>{lines.length > 0 && <Table head={["Sản phẩm", "SL", "Đơn giá", "Thành tiền", ""]}>{lines.map((l) => { const p = visProducts.find((x) => x.id === l.sku); return <Row key={l.sku}><Td>{p.name}</Td><Td><input type="number" min="1" value={l.qty} onChange={(e) => upd(l.sku, "qty", e.target.value)} className="w-16 px-2 py-1 border rounded-lg" /></Td><Td>{fmt(l.price)}</Td><Td className="font-semibold">{fmt(l.qty * l.price)}</Td><Td><button onClick={() => setLines(lines.filter((x) => x.sku !== l.sku))} className="text-rose-500 p-1.5"><Trash2 size={16} /></button></Td></Row>; })}</Table>}<div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><Input label="Giảm giá" type="number" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} /><Input label="Khách trả" type="number" value={paid} onChange={(e) => setPaid(Number(e.target.value))} /></div><div className="flex items-center justify-between pt-4 border-t border-slate-100 flex-wrap gap-3"><span className="text-lg font-bold">Tổng: <span className="text-blue-600">{fmt(total)}</span></span><Btn onClick={save}><CheckCircle2 size={16} /> Lưu đơn</Btn></div></Card></div>;
};

/* ============ SẢN PHẨM ============ */
const Products = () => {
  const { visProducts, setProducts, branches, stock, nav, toast, isAdmin, canSeeCost } = useStore();
  const [q, setQ] = useState(""); const [fBranch, setFBranch] = useState("all"); const [detail, setDetail] = useState(null);
  const { stockHistory } = useStore();
  ...
  return <div><PageHead title="Danh sách sản phẩm" desc={`${list.length} sản phẩm`} action={<Btn onClick={() => nav("product-new")}><Plus size={16} /> Tạo sản phẩm</Btn>} />
...
const InboundExcel = () => {
  ...
  const COL = { name: ["tên sản phẩm", "tên album", "name"], sku: ["sku", "mã sku"], barcode: ["barcode", "mã vạch"], desc: ["mô tả", "desc"], group: ["loại sản phẩm", "loại", "thể loại"], brand: ["nhãn hiệu", "brand", "hãng"], cost: ["giá nhập", "giá vốn", "cost"], price: ["giá bán", "giá bán lẻ", "price"] };
  const cols = ["Tên sản phẩm *", "Mã SKU", "Barcode", "Mô tả", "Loại sản phẩm", "Nhãn hiệu", "Giá nhập", "Giá bán"];
  const fk = (h, v) => h.find((x) => v.includes(String(x).trim().toLowerCase()));
  const parse = (file) => { if (!file || !XLSX) return; if (!/\.(xlsx|xls|csv)$/i.test(file.name)) return toast("Chọn file Excel!"); setLoading(true); setFileName(file.name); const r = new FileReader(); r.onload = (e) => { try { const wb = XLSX.read(e.target.result, { type: "array" }); const json = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval: "" }); if (!json.length) { toast("File rỗng!"); setLoading(false); return; } const H = Object.keys(json[0]); const K = {}; Object.entries(COL).forEach(([k, v]) => (K[k] = fk(H, v))); if (!K.name) { toast("Thiếu cột bắt buộc: Tên sản phẩm"); setLoading(false); return; } let auto = products.length + 1; const P = [], E = []; json.forEach((row, i) => { const name = String(row[K.name] || "").trim(); let sku = K.sku ? String(row[K.sku] || "").trim() : ""; const isAuto = !sku; if (!sku) sku = "SKU" + String(auto++).padStart(3, "0"); const rec = { id: sku, name, artist: "", brand: K.brand ? String(row[K.brand] || "").trim() : "", barcode: K.barcode ? String(row[K.barcode] || "").trim() : "", desc: K.desc ? String(row[K.desc] || "").trim() : "", group: K.group ? String(row[K.group] || GROUPS[0]).trim() : GROUPS[0], cost: Number(String(row[K.cost]).replace(/[^\d.-]/g, "")) || 0, price: Number(String(row[K.price]).replace(/[^\d.-]/g, "")) || 0, _row: i + 2, _valid: !!name, _auto: isAuto, _isNew: !products.find((p) => p.id === sku) }; P.push(rec); if (!name) E.push(`Dòng ${i + 2}: thiếu tên sản phẩm`); }); setRows(P); setErrors(E); setLoading(false); toast(`Đã đọc ${P.length} dòng!`); } catch { toast("Lỗi đọc file!"); setLoading(false); } }; r.readAsArrayBuffer(file); };
  const doImport = () => { if (!supplier) return toast("Chọn nhà cung cấp!"); const v = rows.filter((r) => r._valid); if (!v.length) return toast("Không có dòng hợp lệ!"); importProducts(v.map((r) => ({ id: r.id, name: r.name, brand: r.brand, barcode: r.barcode, desc: r.desc, group: r.group, cost: r.cost, price: r.price, branchId: br })), br, supplier); toast(`Import ${v.length} sản phẩm!`); setRows([]); setFileName(null); setErrors([]); };
  const dl = () => { if (!XLSX) return; const ws = XLSX.utils.json_to_sheet([{ "Tên sản phẩm": "OK Computer", "Mã SKU": "", "Barcode": "8935101", "Mô tả": "Album 1997", "Loại sản phẩm": "Rock", "Nhãn hiệu": "Parlophone", "Giá nhập": 600000, "Giá bán": 990000 }]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "DiaThan"); XLSX.writeFile(wb, "mau_import.xlsx"); toast("Đã tải file mẫu!"); };
  ...
  return <div className="max-w-5xl"><PageHead title="Nhập hàng mới (Excel)" desc="Mã SKU để trống → hệ thống tự cấp" /><Card className="p-5 mb-4"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end"><div className="flex items-end gap-2"><div className="flex-1"><Select label="Nhà cung cấp *" value={supplier} onChange={(e) => setSupplier(e.target.value)}><option value="">-- Chọn NCC --</option>{suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select></div><Btn variant="outline" onClick={() => setShowAdd(true)}><Plus size={16} /></Btn></div><Select label="Chi nhánh nhập" value={br} onChange={(e) => setBr(e.target.value)}>{branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</Select></div></Card>
    ...
    {rows.length > 0 && <div className="mt-4"><Table head={["Dòng", "SKU", "Tên SP", "Nhãn hiệu", "Loại", "Giá bán", "Trạng thái"]}>{rows.map((r, i) => <Row key={i}><Td className="text-slate-400">{r._row}</Td><Td className="font-bold text-blue-600">{r.id} {r._auto && <span className="text-[10px] text-cyan-500">(auto)</span>}</Td><Td>{r.name || "—"}</Td><Td className="text-slate-500">{r.brand || "—"}</Td><Td className="text-slate-500">{r.group}</Td><Td className="font-semibold">{fmt(r.price)}</Td><Td>{!r._valid ? <Badge cls="bg-rose-50 text-rose-600 ring-rose-200">Lỗi</Badge> : r._isNew ? <Badge cls="bg-cyan-50 text-cyan-600 ring-cyan-200">Tạo mới</Badge> : <Badge cls="bg-slate-100 text-slate-600 ring-slate-200">Cập nhật</Badge>}</Td></Row>)}</Table></div>}
...
const Transfer = () => {
  ...
  const [sku, setSku] = useState(""); const [from, setFrom] = useState(""); const [to, setTo] = useState(""); const [qty, setQty] = useState(1);
  const go = () => { if (!sku || !from || !to || from === to) return toast("Chọn SP & 2 chi nhánh khác nhau!"); if ((stock[`${sku}-${from}`] || 0) < qty) return toast("Không đủ hàng!"); doTransfer(sku, from, to, Number(qty)); toast("Chuyển hàng thành công!"); };
  return <div className="max-w-2xl"><PageHead title="Chuyển hàng giữa chi nhánh" /><Card className="p-5 sm:p-6 space-y-4"><Select label="Sản phẩm" value={sku} onChange={(e) => setSku(e.target.value)}><option value="">-- Chọn --</option>{visProducts.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</Select><div className="grid grid-cols-2 gap-4"><Select label="Từ chi nhánh" value={from} onChange={(e) => setFrom(e.target.value)}><option value="">--</option>{branches.map((b) => <option key={b.id} value={b.id}>{b.name} ({sku ? stock[`${sku}-${b.id}`] || 0 : "-"})</option>)}</Select><Select label="Đến chi nhánh" value={to} onChange={(e) => setTo(e.target.value)}><option value="">--</option>{branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</Select></div><Input label="Số lượng" type="number" min="1" value={qty} onChange={(e) => setQty(e.target.value)} /><div className="flex justify-end"><Btn onClick={go}><ArrowRightLeft size={16} /> Chuyển</Btn></div></Card></div>;
};
...
const Customers = () => {
  ...
  return <div><PageHead title="Danh mục khách hàng" desc={`${customers.length} khách`} action={<Btn onClick={() => nav("customer-new")}><Plus size={16} /> Tạo mới</Btn>} /><BulkBar count={sel.length} onDelete={del} onClear={() => setSel([])} /><Table head={[<Check checked={allChecked} onChange={toggleAll} />, "Mã KH", "Tên", "SĐT", "Nhóm", "Tổng đã mua", "Công nợ"]}>{customers.map((c) => <Row key={c.id}><Td><Check checked={sel.includes(c.id)} onChange={() => toggle(c.id)} /></Td><Td className="font-bold text-blue-600">{c.id}</Td><Td className="font-medium">{c.name}</Td><Td className="text-slate-500">{c.phone}</Td><Td><Badge cls={gb[c.group]}>{c.group}</Badge></Td><Td className="font-semibold text-emerald-600">{fmt(bought(c.id))}</Td><Td className={c.debt > 0 ? "text-rose-600 font-medium" : "text-slate-400"}>{fmt(c.debt)}</Td></Row>)}</Table></div>;
};
const CustomerNew = () => {
  const { customers, setCustomers, toast, nav, me } = useStore();
  const [f, setF] = useState({ name: "", phone: "", email: "", address: "", group: "Mới" });
  const save = () => { if (!f.name) return toast("Nhập tên!"); const id = "KH" + String(customers.length + 1).padStart(2, "0"); setCustomers([...customers, { id, ...f, debt: 0, branchId: me.branchId }]); toast("Đã tạo " + id); nav("customers"); };
  return <div className="max-w-2xl"><PageHead title="Tạo khách hàng mới" /><Card className="p-5 sm:p-6 grid grid-cols-1 sm:grid-cols-2 gap-4"><Input label="Tên *" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /><Input label="Điện thoại" value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} /><Input label="Email" value={f.email} onChange={(e) => setF({ ...f, email: e.target.value })} /><Select label="Nhóm" value={f.group} onChange={(e) => setF({ ...f, group: e.target.value })}><option>Mới</option><option>Thân thiết</option><option>VIP</option></Select><div className="sm:col-span-2"><Input label="Địa chỉ" value={f.address} onChange={(e) => setF({ ...f, address: e.target.value })} /></div><div className="sm:col-span-2 flex justify-end gap-3"><Btn variant="ghost" onClick={() => nav("customers")}>Hủy</Btn><Btn onClick={save}>Lưu</Btn></div></Card></div>;
};
const Suppliers = () => {
  ...
  return <div><PageHead title="Danh mục nhà cung cấp" desc={`${suppliers.length} NCC`} action={<Btn onClick={() => nav("supplier-new")}><Plus size={16} /> Tạo mới</Btn>} /><BulkBar count={sel.length} onDelete={del} onClear={() => setSel([])} /><Table head={[<Check checked={allChecked} onChange={toggleAll} />, "Mã", "Tên", "SĐT", "Tổng đã nhập", "Công nợ"]}>{suppliers.map((s) => <Row key={s.id}><Td><Check checked={sel.includes(s.id)} onChange={() => toggle(s.id)} /></Td><Td className="font-bold text-blue-600">{s.id}</Td><Td className="font-medium">{s.name}</Td><Td className="text-slate-500">{s.phone}</Td><Td className="font-semibold text-emerald-600">{fmt(bought(s.id))}</Td><Td className={s.debt > 0 ? "text-rose-600 font-medium" : "text-slate-400"}>{fmt(s.debt)}</Td></Row>)}</Table></div>;
};
const SupplierNew = () => {
...
export default function App() {
  ...
  const [inbounds, setInbounds] = useState(seedInbounds);
  const [sales, setSales] = useState(seedSales);
  const [stock, setStock] = useState(seedStock);
  const [employees, setEmployees] = useState(seedEmployees);
  const [stockHistory, setStockHistory] = useState(seedHistory);
  ...
  const seeAll = isAdmin || myBranch?.central; // Kho Trung Tâm hoặc Admin thấy tất cả
