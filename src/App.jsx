...
/* ============ SẢN PHẨM ============ */
const Products = () => {
  const { visProducts, setProducts, branches, stock, nav, toast, isAdmin, canSeeCost, canEdit, stockHistory, me } = useStore();
  ...
  const viewBranch = isAdmin ? fBranch : me.branchId; // Sales chỉ xem chi nhánh mình
  if (viewBranch !== "all") list = list.filter((p) => (stock[`${p.id}-${viewBranch}`] || 0) > 0);
  const stockAt = (sku) => viewBranch === "all" ? Object.entries(stock).filter(([k]) => k.startsWith(sku + "-")).reduce((a, [, v]) => a + v, 0) : (stock[`${sku}-${viewBranch}`] || 0);
  ...
  return <div><PageHead title="Danh sách sản phẩm" desc={`${list.length} sản phẩm${grouped ? ` · ${artistNames.length} nghệ sĩ` : ""}`} action={canEdit && <Btn onClick={() => nav("product-new")}><Plus size={16} /> Tạo sản phẩm</Btn>} />
    ...
    {!grouped && <>
    {canEdit && <BulkBar count={sel.length} onDelete={delSel} onClear={() => setSel([])} />}
    <Table head={[...(canEdit ? [<Check checked={allChecked} onChange={toggleAll} />] : []), "Ảnh", "SKU", "Nghệ sĩ", "Album", "Nhãn hiệu", ...(isAdmin ? ["Chi nhánh"] : []), ...(canSeeCost ? ["Giá nhập"] : []), "Giá bán", "Tồn", ""]}>
      {list.map((p) => <Row key={p.id}>{canEdit && <Td><Check checked={sel.includes(p.id)} onChange={() => toggle(p.id)} /></Td>}<Td><img src={p.img} alt="" className="w-9 h-9 rounded-lg object-cover" /></Td><Td><button onClick={() => setDetail(p)} className="font-bold text-blue-600 hover:underline">{p.id}</button></Td><Td><button onClick={() => { setQ(p.artist); setGrouped(true); setOpenArtist([p.artist]); }} className="text-left font-medium text-slate-700 hover:text-blue-600 hover:underline">{p.artist || "—"}</button></Td><Td><button onClick={() => setDetail(p)} className="text-left text-slate-800 font-medium hover:text-blue-600 hover:underline">{p.name}</button></Td><Td className="text-slate-500">{p.brand}</Td>{isAdmin && <Td className="text-slate-500">{bName(p.branchId)}</Td>}{canSeeCost && <Td className="text-slate-600">{fmt(p.cost)}</Td>}<Td className="font-semibold">{fmt(p.price)}</Td><Td className="text-slate-700">{stockAt(p.id)}</Td><Td><div className="flex gap-1"><button onClick={() => setDetail(p)} className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600"><Eye size={16} /></button>{canEdit && <button onClick={() => setEdit({ ...p })} className="p-1.5 rounded-lg hover:bg-amber-50 text-amber-600"><Pencil size={16} /></button>}</div></Td></Row>)}
    </Table></>}
...
// 1) Tải toàn bộ dữ liệu từ Supabase khi mở app
  ...
  const visProducts = products; // Danh mục đĩa dùng chung mọi chi nhánh (tồn kho vẫn tách theo CN)
